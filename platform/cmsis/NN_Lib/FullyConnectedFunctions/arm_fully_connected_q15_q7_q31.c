                                          ...
                                          q15_t *pOut)
{
    ...
    if (bias){
        ...
        while (rowCnt)
        {
            ...
            q31_t sum2 = (q31_t)(*pBias++) + NN_ROUND(out_shift);
        ...
        while (rowCnt)
        {
            q31_t sum = (q31_t)NN_ROUND(out_shift);
            q31_t sum2 = (q31_t)NN_ROUND(out_shift);
            uint16_t colCnt = dim_vec >> 2;
    ...
    if (bias){
        ...
        {
            int ip_out = (q31_t)(bias[i]) + NN_ROUND(out_shift);
            for (j = 0; j < dim_vec; j++)
            {
        ...
        for (i = 0; i < num_of_rows; i++)
        {
            int ip_out = (q31_t)NN_ROUND(out_shift);
            for (j = 0; j < dim_vec; j++)
            {
                ip_out += pV[j] * pM[i * dim_vec + j];
            }
