...
{
    if (p->name)
        TRACE(1,"    Timer: %s", p->name);
    else
        TRACE(1,"    Timer Callback: %p", p->finfo.func);

    if (p->state >= 0 && p->state < ARRAY_SIZE(timer_states))
        TRACE(1,"    Timer state: %s", timer_states[p->state]);
    if (p->type >= 0 && p->type < ARRAY_SIZE(timer_types))
        TRACE(1,"    Timer type %s", timer_types[p->type]);
    TRACE(2,"    Timer current tick %u, Load %u\n", p->tick, p->load);
}
...
void rtx_show_timer_stats(void)
{
    ...
    if (handling_timers > 0) {
        int i;
        TRACE(1,"--- Timers is handleing:%d", handling_timers);
        for (i = 0; i < handling_timers; i++) {
            ...
            status = osMessageQueueGet(osRtxInfo.timer.mq, &finfo, NULL, osWaitForever);
            if (status == osOK) {
                TRACE(1, "    Handling Timer callback %p", finfo.func);
            }
        }
    }
}
