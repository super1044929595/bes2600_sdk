                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    TRACE(2,"gattc_write_req_ind_handler write handle %d start hdl %d", param->handle, gfpsp_env->start_hdl);
    ...
    if (gfpsp_env != NULL)
    {
        switch (param->handle-gfpsp_env->start_hdl)
        {
...
#ifdef SPOT_ENABLED
            case GFPSP_IDX_BEACON_ACTIONS_VAL:
            {
                TRACE(0,"beacon actions");
                DUMP8("%02x ", param->value, param->length);
            ...
            case GFPSP_IDX_BEACON_ACTIONS_CFG:
            {
                uint16_t value = 0x0000;            
                memcpy(&value, &(param->value), sizeof(uint16_t));
                struct app_gfps_pass_key_notif_config_t * ind = KE_MSG_ALLOC(GFPSP_BEACON_ACTIONS_NTF_CFG,
                        prf_dst_task_get(&gfpsp_env->prf_env, conidx),
                        prf_src_task_get(&gfpsp_env->prf_env, conidx),
                        app_gfps_pass_key_notif_config_t);
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    TRACE(2,"gattc_read_req_ind_handler read handle %d start hdl %d", param->handle, gfpsp_env->start_hdl);

    if (param->handle == (gfpsp_env->start_hdl + GFPSP_IDX_KEY_BASED_PAIRING_NTF_CFG))
    {
        uint16_t notify_ccc;
...
#ifdef SPOT_ENABLED
    else if(param->handle == (gfpsp_env->start_hdl + GFPSP_IDX_BEACON_ACTIONS_CFG)) 
    {
        uint16_t notify_ccc;
        cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(notify_ccc));
    ...
    else if(param->handle == (gfpsp_env->start_hdl + GFPSP_IDX_BEACON_ACTIONS_VAL)) 
    {
        uint8_t nonce[9];
        ...
        cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(nonce));

        cfm->length = dataLen;
        memcpy(cfm->value, nonce, cfm->length);
    ...
    else
    {
        ...
        cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, dataLen);
        cfm->length = dataLen;
    }
