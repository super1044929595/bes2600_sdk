...
typedef struct {
	...
	int         pwr_window_size;
    /*
    weight[0]: freq[0] ~ freq[1]
    weight[1]: freq[1] ~ freq[2]
    weight[2]: freq[2] ~ freq[3]
    */
    float       band_freq[4];
    float       band_weight[3];
} assist_noise_cfg_t;
...
AncAssistState *anc_assist_create(int sample_rate, int sample_bits, int ch_num, int frame_size, const AncAssistConfig *cfg, anc_assist_notify_func_t callback,uint8_t * heap_buf, uint32_t heap_size);
...
int32_t anc_assist_get_prompt_anc_index(AncAssistState *st,int * default_anc_index,int channel_idx);
