...
typedef struct {
    ...
    bool        xos_init_enable;
}xos_Shell_List;

static xos_Shell_List xos_ShellListInfo={
	
.xos_shell_active_num =0,
.xos_shell_mailboxid  =NULL,
.xos_shell_threadid	  =NULL,
.xos_shell_mempoolid  =NULL,
.xos_init_enable	  =0
...
static bool xOS_ui_shell_handle(XOS_Shell_UI_Mod_E mod,uint8_t *pdata,uint16_t len)
{
	xos_shell_debug("xos xOS_ui_shell_handle [MODULE:%d] led:%d",mod,len);
...
bool xos_ui_shell_register(XOS_Shell_UI_Mod_E xshell_mod,xos_ui_shell_handle_typedef xshell_app)
{
	...
	if(xos_ShellListInfo.xos_init_enable==0){
		xos_ui_shell_init();
		xos_ShellListInfo.xos_init_enable=1;
	}
